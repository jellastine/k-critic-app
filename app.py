...
def save_report_to_pdf(game_name, predicted_score, summary, unique_features, main_elements, competitors):
    ...
    pdf.cell(200, 10, txt=f"K-크리틱 분석 리포트 - {game_name}".encode('latin-1', 'replace').decode('latin-1'), ln=True, align='C')
    pdf.ln(10)

    body = f"예상 메타크리틱 점수: {predicted_score:.1f}\n\n문서 요약:\n{summary}\n\n강점 요약:\n- 차별화 포인트: {unique_features}\n- 핵심 시스템: {main_elements}\n\n경쟁작:\n{competitors}"
    body_encoded = body.encode('latin-1', 'replace').decode('latin-1')
    pdf.multi_cell(0, 10, txt=body_encoded)
